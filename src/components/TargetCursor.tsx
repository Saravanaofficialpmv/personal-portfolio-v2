...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const cornersRef = useRef<HTMLElement[] | null>(null);
  ...
  useEffect(() => {
    ...
    const checkSectionHover = (targetEl: HTMLElement | null) => {
      if (!sectionSelector) return true;
      if (!targetEl) return false;
      const sectionEl = document.querySelector(sectionSelector);
      if (!sectionEl) return false;
      return sectionEl.contains(targetEl) || targetEl === sectionEl;
    };
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      if (sectionSelector) {
        const inside = checkSectionHover(e.target as HTMLElement | null);
        if (inside && !isInsideSection) {
        ...
        } else if (!inside && isInsideSection) {
